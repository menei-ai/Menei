...
/// AUM Zero, pension edition. The asset manager where the law itself ages.
///
...
contract AUM0Pension {
    ...
    function _targetNow(Account storage acct) internal view returns (uint16[] memory t) {
        t = acct.targetBps;
        uint256 n = t.length;
        if (n == 0 || acct.endBps.length == 0) return t;
        if (block.timestamp <= acct.glideStart) return t;
        if (block.timestamp >= acct.glideEnd) return acct.endBps;
        uint256 num = block.timestamp - acct.glideStart;
        uint256 den = acct.glideEnd - acct.glideStart;
        uint256 sum;
        for (uint256 i = 1; i < n; ++i) {
            uint256 v = (uint256(acct.targetBps[i]) * (den - num) + uint256(acct.endBps[i]) * num) / den;
            t[i] = uint16(v);
            sum += v;
        }
