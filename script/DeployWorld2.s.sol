...
/// The world edition, and bitcoin joins the venue: USDG plus the same
/// twenty six assets in the same order, and cbBTC at the end, priced by the
/// official CBBTC / USD feed. Stocks and bitcoin under one law, frozen at
/// deploy forever.
contract DeployWorld2 is Script {
    address constant ROUTER = 0xCaf681a66D020601342297493863E78C959E5cb2;
    address constant USDG   = 0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168;
    uint256 constant N = 27;

    function run() external {
        vm.startBroadcast();
        AUM0World aum = _deploy();
        vm.stopBroadcast();
        console2.log("AUM0 world edition, bitcoin aboard, deployed at:", address(aum));
    }

    function deployForTest() external returns (AUM0World) {
        return _deploy();
    }

    function _deploy() internal returns (AUM0World) {
        ...
        // and the first coin: eight decimals, a live official feed, served by
        // the desk and by inventory until its pool grows up
        _set(tokens, feeds, fees, 26, 0xCEC185eB182c47d1bA1EFc84e6959e18cd620Be4, 0x3546407C5F94dD7Eab3a853D245b5C5EAb53318a, 500);   // cbBTC

        for (uint256 i; i < N; ++i) decs[i] = 18;
        decs[26] = 8;
        return new AUM0World(ROUTER, USDG, 6, 7 days, tokens, feeds, fees, decs);
    }
