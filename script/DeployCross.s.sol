...
/// The desk edition on the full venue: USDG plus the same twenty six assets
/// as the bid edition, frozen at deploy forever. Everything the bid edition
/// does, plus the internal cross.
contract DeployCross is Script {
    ...
    function run() external {
        vm.startBroadcast();
        AUM0Cross aum = _deploy();
        vm.stopBroadcast();
        console2.log("AUM0 desk edition deployed at:", address(aum));
    }

    function deployForTest() external returns (AUM0Cross) {
        return _deploy();
    }

    function _deploy() internal returns (AUM0Cross) {
